...
function lo_plugin_activate()
{
	...
	$wpdb->query("CREATE TABLE IF NOT EXISTS `lo_optin_forms` (
...
function lo_optin_form_shortcode($atts) {
	...
	$form_data = $wpdb->get_row($wpdb->prepare("SELECT `".$wpdb->escape($type)."` FROM `lo_optin_forms` WHERE `id`=%d",$id),ARRAY_A);
...
function lo_admin_lead_track_convert_options()
{
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'update')
	{
		...
		{
			$perform_update = true;
			$posts_update_success = update_option( 'lo_lead_track_convert_posts', ($_POST['lead_track_convert_posts'] == '1' ? '1' : '0') );
			$pages_update_success = update_option( 'lo_lead_track_convert_pages', ($_POST['lead_track_convert_pages'] == '1' ? '1' : '0') );
		}
	}
...
function lo_admin_optin_forms()
{
	...
	elseif(isset($_REQUEST['action']) && $_REQUEST['action'] == 'edit' && isset($_REQUEST['form_id']) && $_REQUEST['form_id'] > 0)
	{
		...
		{
			$perform_update = true;
			$update_success = $wpdb->update( 'lo_optin_forms'
					, array(
...
function lo_delete_optin_form($ids)
{
	...
	{
		$wpdb->query($wpdb->prepare("DELETE FROM `lo_optin_forms` WHERE `id` = %d",$ids[$a]));
	}
	?>
